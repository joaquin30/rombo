...
(fn init []
  ...
  (set tilemap (newTilemap (require "map.3") world))
  (set player (newPlayer 1 1 world))
  (set star (newStar 45 17 world))
  (set texts [])
  (lume.push texts
    (newText "Agarra la estrella\ny mira que pasa" _G.font_normal [0 0 0] -12 -3)
    (newText "Siempre puedes\n\"volver atras\"\npresionando Z" _G.font_normal [0 0 0] 12 -5)))
...
(fn update []
  ...
  ;escenas
  (if
    (> player.y _G.height) "menu"
    player.dead "tutorial3"))
